...
+ (NSPushConfigSnapshot *)loadSnapshot {
  ...
  NSArray *globalAppList = getAppIDsWithPrefix(prefs, NSPPreferenceGlobalBLPrefix);
  ...
  for (NSString *service in
       [[customServices allKeys] sortedArrayUsingSelector:@selector(compare:)]) {
  ...
  return [NSPushConfigSnapshot
      snapshotWithEnabled:enabled
