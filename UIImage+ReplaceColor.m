...
- (UIImage*)imageByReplacingColor:(UIColor*)sourceColor
  ...
  for (int byte = 0; byte < bytesPerRow * height; byte += 4) {
    ...
    if (ratio > 0.1)
      ratio = 1; // if ratio is too far away, set it to max.
    if (ratio < 0)
      ratio = 0; // if ratio isn't far enough away, set it to min.

    // blend color components
    data[byte] = (UInt8)round(ratio * r) +
                 (UInt8)round((1.0 - ratio) * target255Components[0]);
    data[byte + 1] = (UInt8)round(ratio * g) +
                     (UInt8)round((1.0 - ratio) * target255Components[1]);
    data[byte + 2] = (UInt8)round(ratio * b) +
                     (UInt8)round((1.0 - ratio) * target255Components[2]);
  }
