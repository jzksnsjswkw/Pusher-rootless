...
- (void)loadPreferences {
  _selectedApplicationID = self.selectedAppID;
}

- (void)savePreferences {
  if (callback) {
    callback(self->_selectedApplicationID);
  }
...
//   NSString *displayIdentifier = [self
//   displayIdentifierForIndexPath:indexPath]; cell.accessoryType =
//   [self.appSelectionController.selectedAppIDs
//                            containsObject:displayIdentifier]
...
//   _appListDataSource.sectionDescriptors =
//       [NSPAppSelectionALApplicationTableDataSource
//       standardSectionDescriptors];
...
//   _searchController.searchBar.tintColor =
//       NSPusherManager.sharedController.activeTintColor;
//   self.tableView.tableHeaderView = _searchController.searchBar;

//   if (!self.selectedAppIDs ||
//       ![self.selectedAppIDs isKindOfClass:NSMutableArray.class]) {
...
//         initWithTitle:(self.rightButtonTitle ?: @"Done")
//                 style:UIBarButtonItemStylePlain
//                target:self
...
// - (void)doneSelecting {
//   if (callback) {
//     callback(self.selectingMultiple
//                  ? self.selectedAppIDs
//                  : (self.selectedAppIDs.count ? self.selectedAppIDs[0] :
//                  nil));
//   }
//   [self dismiss];
// }
...
//       [NSPAppSelectionALApplicationTableDataSource
//               .standardSectionDescriptors mutableCopy];
//   if (XIsEmpty(filter)) {
...
//       NSMutableDictionary *filteredSectionDescriptor =
//           [sectionDescriptor mutableCopy];
//       filteredSectionDescriptor[ALSectionDescriptorPredicateKey] = XStr(
//           @"%@ AND displayName CONTAINS[cd] '%@'",
//           filteredSectionDescriptor[ALSectionDescriptorPredicateKey],
//           filter);
//       // XLog(@"filteredSectionDescriptor[ALSectionDescriptorPredicateKey] =
//       // %@", filteredSectionDescriptor[ALSectionDescriptorPredicateKey]);
...
//   [self.tableView
//         reloadSections:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0,
//         2)]
...
//       [_appListDataSource displayIdentifierForIndexPath:indexPath];
//   if (self.selectingMultiple) {
