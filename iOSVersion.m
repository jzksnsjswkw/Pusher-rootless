...
// Mine
#define SYSTEM_VERSION_IS_IN_RANGE(v, w)                                       \
  (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(v) &&                               \
   SYSTEM_VERSION_LESS_THAN_OR_EQUAL_TO(w))
 ...
 * Checks if version is any iOS 8 version. (Use .9.9 to include ALL of Apple's
future releases of this version) if(SYSTEM_VERSION_IS_IN_RANGE(@"8.0",
@"8.9.9")) {
    ...
}
 ...
 * Checks if version is anything except 8.0, 8.0.1, 8.0.2, 8.1, and 8.1.1
(anything inside range of 8.0 - 8.1.1) if (SYSTEM_VERSION_NOT_IN_RANGE(@"8.0",
@"8.1.1")) {
    ...
}
